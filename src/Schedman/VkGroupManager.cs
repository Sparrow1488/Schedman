...
namespace Schedman
{
    public class VkGroupManager : GroupManager<VkPublishEntity>
    {
        ...
        public override async Task<PublishResult> PublishAsync(VkPublishEntity post)
        {
            var publishExceptions = new List<SchedmanException>();
            long postId = 0;
            ...
            while (attemptCurrent < attemptMax)
            {
                ...
                {
                    publishExceptions.Add(new SchedmanFailedGetResponseException(ex.Message));
                }
                finally
                {
                    if (postId != 0)
            ...
            return new PublishResult(publishExceptions, Array.Empty<VkPublishEntity>());
        }
